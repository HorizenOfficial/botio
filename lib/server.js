...
//
// Setup Git credential manager if using a private repo
//
if (config.private_repo) {
    console.log('private_repo config flag detected, caching github creds.');
    var result = shell.exec("TDIR=$(mktemp -d) && \
    echo https://" + global.user + ":" + global.pwd + "@github.com > $TDIR/gitcreds && \
    git config --global credential.helper 'store --file $(echo $TDIR)/gitcreds'", {silent:true});
    ...
    function maybeEnqueueJob(payload, pull, event) {
      var id = randomHash(15),
          jobInfo = {
            ...
            public_url: 'https://'+config.host+'/'+id,
            base_url: 'https://github.com/'+config.repo+'.git',
            base_ref: pull.base.ref,
            head_url: pull.head.repo.clone_url,
